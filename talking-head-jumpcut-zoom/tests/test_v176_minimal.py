...
def observations(duration_ms=18000):
    return [
        {
            "t_ms": t,
            "face_ratio": 0.22,
...
def base_payload(duration_ms=12000, events=None):
    return {
        "source": {
            "width": 1080,
            "height": 1920,
...
class MinimalV176Tests(unittest.TestCase):
    def test_semantic_event_adds_block_accent_and_stable_duration(self):
        ...
        self.assertEqual(
            built["semantic_duration_ms"],
            words[6]["end_ms"] - words[2]["start_ms"],
        )
    ...
    def test_semantic_contract_requires_block_and_accent(self):
        ...
        self.assertEqual(
            result["semantic_events"][0]["semantic_contract"], "legacy_fallback"
        )
    ...
    def test_four_zoom_levels_are_bounded_at_113(self):
        ...
        self.assertEqual(
            _semantic_level(
                {"state": "ARGUMENT", "importance": 0.45, "semantic_importance": 0.45}
            ),
            "Z1",
        )
        self.assertEqual(
            _semantic_level(
                {"state": "ARGUMENT", "importance": 0.60, "semantic_importance": 0.60}
            ),
            "Z2",
        )
        self.assertEqual(
            _semantic_level(
                {"state": "ARGUMENT", "importance": 0.75, "semantic_importance": 0.75}
            ),
            "Z3",
        )
        self.assertEqual(
            _semantic_level(
                {"state": "EMPHASIS", "importance": 0.95, "semantic_importance": 0.95}
            ),
            "Z4",
        )
    ...
    def test_performance_bonus_alone_cannot_create_z4(self):
        self.assertEqual(
            _semantic_level(
                {"state": "EMPHASIS", "importance": 0.88, "semantic_importance": 0.80}
            ),
            "Z3",
        )
        self.assertEqual(
            _semantic_level(
                {"state": "EMPHASIS", "importance": 0.91, "semantic_importance": 0.91}
            ),
            "Z4",
        )
        self.assertEqual(
    ...
    def test_safe_boundary_keeps_semantic_duration_but_visual_dwell_is_three_seconds(self):
        result = plan(
            base_payload(
                7000,
                [
        ...
        self.assertEqual(decision["zoom_duration_ms"], 3000)
        self.assertEqual(decision["end_ms"], 4400)
        ...
        result = plan(
            base_payload(
                ...
                [
                    ...
                    event("e2", 4200, 0.65, block_id="same"),
                ],
            )
        )
        ...
        result = plan(
            base_payload(
                ...
                [
                    ...
                    event("e2", 5500, 0.65, block_id="b", duration_ms=800),
                ],
            )
        )
        self.assertGreaterEqual(result["short_home_flashes_suppressed"], 1)
        self.assertFalse(
            [r for r in result["returns"] if r.get("parent_event_id") == "e1"]
        )

    def test_cadence_starts_with_z1_and_never_exceeds_z2(self):
        result = plan(base_payload(14000))
    ...
    def test_semantic_z3_at_four_seconds_suppresses_preceding_cadence(self):
        result = plan(
            base_payload(
                9000,
                [event("punch", 4000, 0.75, block_id="b1", duration_ms=1200)],
            )
        )
        cadence_before = [
            d
        ...
        punch = next(
            d for d in result["decisions"] if d.get("event_id") == "punch"
        )
        ...
        result = plan(
            base_payload(
                ...
                [
                    ...
                    event(
                        "punch", 4200, 0.75, block_id="arc", duration_ms=1600
                    ),
                    event(
                        "peak",
                        7400,
    ...
    def test_build_automatically_uses_about_two_second_slow_push(self):
        result = plan(
            base_payload(
                9000,
                [
        ...
        push = next(d for d in result["decisions"] if d.get("event_id") == "build")
        self.assertEqual(push["motion"], "slow_push")
        self.assertTrue(push.get("auto_slow_push"))
        self.assertEqual(push["transition_end_ms"] - push["start_ms"], 2000)
        self.assertGreaterEqual(push["end_ms"] - push["transition_end_ms"], 500)

    def test_explicit_slow_push_has_settle_or_falls_back_to_step(self):
        result = plan(
            base_payload(
                9000,
                [
    ...
    def test_qc_enforces_global_three_second_framing_gap(self):
        result = plan(base_payload(14000))
        report = check(result)
        ...
        bad = {
            "version": "1.7.6-lite",
            "source": {"width": 1080, "height": 1920, "duration_ms": 6000},
            ...
            "decisions": [
                ...
                },
                {
                    ...
                    "zoom_level": "Z2",
                    "scale": 1.06,
                    "crop_start": [16, 28, 1048, 1864],
                    "crop_end": [30, 54, 1018, 1812],
                },
            ],
            "returns": [],
        }
        ...
        self.assertIn(
            "framing_change_too_fast",
            {e["check"] for e in bad_report["errors"]},
        )
    ...
    def test_rhythm_summary_reports_level_counts_and_min_gap(self):
        ...
        self.assertGreaterEqual(
            summary["minimum_gap_between_framing_changes_ms"], 3000
        )
        self.assertEqual(summary["cadence_preferred_gap_ms"], 4500)
        self.assertEqual(summary["cadence_max_gap_ms"], 6000)
